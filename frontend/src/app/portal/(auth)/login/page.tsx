...
function PortalLoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setIsLoading(true);
    try {
      const userType = await login({ email: form.email.trim(), password: form.password }, "portal");
    ...
    <div className="min-h-screen bg-fog flex">
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <h1 className="text-[28px] font-bold font-display text-ink mb-1.5">Student & Parent portal</h1>
          <p className="text-sm text-mid mb-8">
            Access is limited to your institution. Students see their own class and ID card; parents see only linked children.
          </p>
          <form className="flex flex-col gap-4" onSubmit={handleSubmit} noValidate>
            ...
            <div className="flex justify-end">
              <Link href="/portal/forgot-password" className="text-xs text-brand hover:underline no-underline">
                Forgot password?
              </Link>
            </div>
