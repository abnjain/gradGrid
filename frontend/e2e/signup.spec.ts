...
test.describe("Signup — application flow", () => {
  test("successful submit advances to email verification step", async ({ page }) => {
    await page.route("**/api/v1/auth/register-institution", (route) =>
      route.fulfill({
        status: 201,
    ...
    await expect(page.locator("body")).toContainText("owner@school.edu");
  ...
  test("API error on submit shows error toast", async ({ page }) => {
    await page.route("**/api/v1/auth/register-institution", (route) =>
      route.fulfill({
        status: 409,
        contentType: "application/json",
    ...
    await expect(page.getByText("already under review")).toBeVisible();
  });
